...
- [ ] Hive journal unencrypted at rest (`R7.8` — backlog severity, bundled into the design phase
  since it touches the same journal feature as `R7.6`)
...
### Compliance
- [ ] Privacy policy unpublishable + unreachable from app (`R6.1`, `R6.2`)
- [ ] No consent/notice at point of collection (`R6.2`)
- [ ] Legal basis for children's data weak (`R6.3`)
- [ ] FCM notification transparency to guides (`R6.4`)
- [ ] Cross-border transfer undocumented (`R6.5`)
- [ ] No Article 28 processor documentation (`R6.6`)
- [ ] Crashlytics retention/erasure undocumented (`R6.7`)
- [ ] Children's photos not distinctly called out (`R6.8`)
- [ ] No breach response plan (`R6.9`)
- [ ] No supervisory authority mention (`R6.10`)
- [ ] `review-notes.md` placeholder credentials (`R6.11`)

### Product Design
- [ ] Hardcoded `Colors.red` in emergency screens (`R7.1`)
- [ ] Map marker touch targets/feedback (`R7.2`)
- [ ] Systemic absence of accessibility semantics (`R7.3`)
- [ ] Kid nav label visibility contradicts DESIGN.md (`R7.4`)
- [ ] Guide nav density vs. text expansion (`R7.4`, same task)
- [ ] Journal photo-remove control undersized/unconfirmed (`R7.5`)
- [ ] Teams-management dialog no loading guard (`R7.6`)
- [ ] Second hardcoded red in `camp_session_screen.dart` (`R7.1`, same task)
- [ ] Spinner-only loading states (`R7.7` — documented as accepted, no action)
