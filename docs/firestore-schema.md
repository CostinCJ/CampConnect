...
|---|---|---|---|
| `users/{uid}` | server-only create (`registerGuide`/`claimCampCode`); client may update only the `campId` field on their own doc | `role` ('guide'\|'kid'), `email?`, `displayName`, `campId?`, `team?`, `orgId?`, `createdAt` | A guide switching `campId` must target a camp in their own org (enforced in rules); kids never write their own profile. |
| `organizations/{orgId}` | `registerGuide` callable only | `name`, `ownerUid`, `inviteCode` | Client writes denied; readable only by guides of that org. |
| `organizations/{orgId}/members/{uid}` | `registerGuide` callable (owner on creation, guide on join) / `deleteMyAccount` (removes non-owner membership) | `role` ('owner'\|'guide'), `displayName` | Client writes denied. |
