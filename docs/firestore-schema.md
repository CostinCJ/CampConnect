...
| `organizations/{orgId}/announcementTemplates/{templateId}` | org-scoped guides (client, via `announcement_templates_repository.dart`); auto-seeded with 8 built-in defaults the first time an org has none | `titles` (map: languageCode -> title), `bodies` (map: languageCode -> body), `order` | Guide-only (rules deny kid read); reusable prewritten announcements a guide can send with one tap, shared across all the org's camps like master locations. Not pushed themselves — sending one creates a normal `camps/{campId}/announcements/{id}` doc. |
| `camps/{campId}` | org-scoped guides (create/update/delete); read by camp members or org guides | `name`, `startDate`, `endDate`, `teams` (list of team names used at creation), `createdBy`, `orgId`, `language`, `tvCode?` | `language` drives the Cloud Functions notification strings ('ro'/'hu', default 'ro'). Note: this `teams` field is a static snapshot of names captured at creation — distinct from the live, mutable `camps/{campId}/teams/{teamId}` subcollection below. `tvCode` is a guide-generated 6-char code (`camp_repository.dart`'s `generateUniqueTvCode`, unique within the org), created on demand from the points management screen; the public `tvLeaderboard` HTTP function looks up a camp by `tvCode` for the unauthenticated TV display (`docs/tv/index.html`, served via GitHub Pages), returning only team name/color/points — never personal data. |
